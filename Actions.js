...
API.onServerEventTrigger.connect(function (eventName, args) {
    switch (eventName) {
        case "ReceiveActions":
            if (actions_menu == null) {
                ...
                for (var i = 0; i < data.length; i++)
                    actions_menu.AddItem(API.createMenuItem(data[i], ""));
                actions_menu.MenuItems[0].SetLeftBadge(BadgeStyle.Tick);
                actions_menu.OnItemSelect.connect(function (menu, item, index) {
        ...
        case "SetCurrentActionIndex":
            if (actions_menu == null)
                return;
            for (var i = 0; i < actions_menu.MenuItems.Count; i++)
                actions_menu.MenuItems[i].SetLeftBadge(BadgeStyle.None);
            actions_menu.MenuItems[args[0]].SetLeftBadge(BadgeStyle.Tick);
            break;
        case "RequestAnimationLength":
            API.loadAnimationDict(args[0]);
            var animTime = API.getAnimTotalTime(args[0], args[1]);
            animTime += 0.0000001;
